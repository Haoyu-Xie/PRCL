...
def attention_threshold_loss(pred: torch.Tensor, pseudo_label: torch.Tensor, logits: torch.Tensor, strong_threshold):
    ...
    weighting = logits.view(batch_size, -1).ge(strong_threshold).sum(-1) / (valid_mask.view(batch_size, -1).sum(-1) + 1e-6)
...
class Attention_Threshold_Loss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, pseudo_label: torch.Tensor, logits: torch.Tensor):
        batch_size = pred.shape[0]
        valid_mask = (pseudo_label >= 0).float() # only count valid pixels (class)
        weighting = logits.view(batch_size, -1).ge(self.strong_threshold).sum(-1) / (valid_mask.view(batch_size, -1).sum(-1) + 1e-8) # May be nan if the whole target is masked in cutout
        # weight represent the proportion of valid pixels in this batch
        loss = F.cross_entropy(pred, pseudo_label, reduction='none', ignore_index=-1) # pixel-wise
        weighted_loss = torch.mean(torch.masked_select(weighting[:, None, None] * loss, loss > 0))
        # weight torch.size([4]) -> weight[:, None, None] torch.size([4, 1, 1]) for broadcast to multiply the weight to the corresponding class
...
class Reco_Loss(nn.Module):
    def __init__(self, strong_threshold=1.0, temp=0.5, num_queries=256, num_negatives=512):
        super(Reco_Loss, self).__init__()
        self.strong_threshold = strong_threshold
        self.temp = temp
        self.num_queries = num_queries
        self.num_negatives = num_negatives

    def forward(self, rep: torch.Tensor, label, mask, prob):
        batch_size, num_feat, rep_w, rep_h = rep.shape
        num_segments = label.shape[1] # num_class
        ...
        for i in range(num_segments): #class
            ...
            if valid_pixel_seg.sum() == 0: # not all classes would be available in a mini-batch
                continue
            prob_seg = prob[:, i, :, :] # class prob torch.size([batch, w, h])
            rep_mask_hard = (prob_seg < self.strong_threshold) * valid_pixel_seg.bool() # select hard queries(prod > threshold & valid mask)
            seg_proto_list.append(torch.mean(rep[valid_pixel_seg.bool()], dim=0, keepdim=True)) # compute mean on batch-dim
            seg_feat_all_list.append(rep[valid_pixel_seg.bool()]) # leave valid feature
            seg_feat_hard_list.append(rep[rep_mask_hard]) #leave valid and confident feat
            seg_num_list.append(int(valid_pixel_seg.sum().item())) # valid pixel num on each class-dim
        ...
        if len(seg_num_list) < 1: # in some rare cases, a small mini-batch only contain 1 or no semantic class
            ...
            for i in range(valid_seg):
                ...
                seg_logits = torch.cosine_similarity(anchor_feat.unsqueeze(1), all_feat, dim=2)
                reco_loss = reco_loss + F.cross_entropy(seg_logits / self.temp, torch.zeros(self.num_queries).long().cuda())
            
            return reco_loss /valid_seg

    def forward(self, rep: torch.Tensor, label, mask, prob):
        return compute_reco_loss(rep, label, mask, prob, self.strong_threshold, self.temp, self.num_queries, self.num_negatives)
...
##### Uncertainty Aware Reco Loss #####

def compute_uareco_loss(rep: torch.Tensor, label, mask, prob, uncertainty_l, uncertainty_un, strong_threshold=1.0, temp=0.5, num_queries=256, num_negatives=256, epoch=0, itert=0):
    batch_size, num_feat, rep_w, rep_h = rep.shape
    num_segments = label.shape[1] # num_class
    device = rep.device
    ...
    uncertainty_un = uncertainty_un.permute(0, 2, 3, 1)
    # Permute prototype (class mean representation) for each class across all valid pixels
    seg_feat_all_list = []
    seg_feat_hard_list = []
    seg_feat_uncertainty_weigh_list = []
    seg_num_list = []
    seg_proto_list = [] # class mean proto
    for i in range(num_segments): #class
        valid_pixel_seg = valid_pixel[:, i] # select binary mask for i-th class torch.size([b, w, h])
        ...
        seg_feat_all_list.append(rep[valid_pixel_seg.bool()]) # leave valid feature
        seg_feat_hard_list.append(rep[rep_mask_hard]) #leave valid and confident feat
        # Uncertainty weigh computing
        bs_half = int(batch_size / 2)
        uncertainty_l_weigh = uncertainty_l[valid_pixel_seg.bool()[:bs_half, ].bool()] #leave valid uncertainty_l 
        uncertainty_un_weigh = uncertainty_un[valid_pixel_seg.bool()[bs_half:, ]] # leave valid uncertainty_un
        if len(uncertainty_un_weigh) != 0:
            uncertainty_un_weigh = F.normalize(1/uncertainty_un_weigh) # inverse uncertainty_un, high uncertainty is risky
        uncertainty_l_weigh = F.normalize(uncertainty_l_weigh)   

        seg_feat_uncertainty_weigh_list.append(torch.cat((uncertainty_l_weigh, uncertainty_un_weigh)))
        seg_num_list.append(int(valid_pixel_seg.sum().item())) # valid pixel num on each class-dim
    ...
    if len(seg_num_list) < 1: # in some rare cases, a small mini-batch only contain 1 or no semantic class
        ...
        for i in range(valid_seg):
            ...
            with torch.no_grad():
                ...
                # index negative keys (from other classes)
                negative_feat_all = torch.cat(seg_feat_all_list[i+1: ] + seg_feat_all_list[: i])
                negative_feat_uncertainty_weigh_all = torch.cat(seg_feat_uncertainty_weigh_list[i+1: ] + seg_feat_uncertainty_weigh_list[: i])
                negative_feat = negative_feat_all[negative_index].reshape(num_queries, num_negatives, num_feat)
                negative_feat_uncertainty_weigh = negative_feat_uncertainty_weigh_all[negative_index].reshape(num_queries, num_negatives, 1)
                # Combine positive and negative keys: keys = [positive key | negative keys] with 1 + num_negative dim
                positive_feat = seg_proto[i].unsqueeze(0).unsqueeze(0).repeat(num_queries, 1, 1)
                uncertainty_positive_weigh = torch.ones(positive_feat.shape[:2]).unsqueeze(-1).to(device)
                all_feat = torch.cat((positive_feat, negative_feat), dim=1)
                all_uncertainty_weigh = torch.cat((uncertainty_positive_weigh, negative_feat_uncertainty_weigh), dim=1)
            
            seg_logits = torch.cosine_similarity(anchor_feat.unsqueeze(1), all_feat, dim=2)
            seg_logits_weighted = all_uncertainty_weigh.squeeze(-1) * seg_logits
            reco_loss = reco_loss + F.cross_entropy(seg_logits_weighted / temp, torch.zeros(num_queries).long().to(device))
        
        return reco_loss /valid_seg

class Uncertainty_Aware_Reco(nn.Module):
    def __init__(self, strong_threshold=1.0, temp=0.5, num_queries=256, num_negatives=256, eps=1.0, scheduler=None) -> None:
        super(Uncertainty_Aware_Reco, self).__init__()
        self.strong_threshold = strong_threshold
        self.temp = temp
        self.num_queries = num_queries
        self.num_negatives = num_negatives
    ...
    def forward(self, rep: torch.Tensor, label, mask, prob, uncertainty_l, uncertainty_un):
        batch_size, num_feat, rep_w, rep_h = rep.shape
        num_segments = label.shape[1]  # num_class
        device = rep.device
        ...
        for i in range(num_segments):  # class
            ...
            seg_proto_list.append(
                torch.mean(rep[valid_pixel_seg.bool()], dim=0, keepdim=True))  # compute mean on batch-dim
            seg_feat_all_list.append(rep[valid_pixel_seg.bool()])  # leave valid feature
            seg_feat_hard_list.append(rep[rep_mask_hard])  # leave valid and confident feat
            # Uncertainty weigh computing
            bs_half = int(batch_size / 2)
            ...
            uncertainty_un_weigh = uncertainty_un[valid_pixel_seg.bool()[bs_half:, ]]  # leave valid uncertainty_un
            if len(uncertainty_un_weigh) != 0:
                uncertainty_un_weigh = F.normalize(
                    1 / (self.eps + uncertainty_un_weigh))  # inverse uncertainty_un, high uncertainty is risky
            uncertainty_l_weigh = 0.5 * F.normalize(uncertainty_l_weigh) + 0.5
        ...
        if len(seg_num_list) < 1:  # in some rare cases, a small mini-batch only contain 1 or no semantic class
            ...
            for i in range(valid_seg):
                ...
                with torch.no_grad():
                    ...
                    # index negative keys (from other classes)
                    negative_feat_all = torch.cat(seg_feat_all_list[i + 1:] + seg_feat_all_list[: i])
                    negative_feat_uncertainty_weigh_all = torch.cat(
                        seg_feat_uncertainty_weigh_list[i + 1:] + seg_feat_uncertainty_weigh_list[: i])
                    negative_feat = negative_feat_all[negative_index].reshape(self.num_queries, self.num_negatives, num_feat)
                    negative_feat_uncertainty_weigh = negative_feat_uncertainty_weigh_all[negative_index].reshape(
                        self.num_queries, self.num_negatives, 1)
                    # Combine positive and negative keys: keys = [positive key | negative keys] with 1 + num_negative dim
                    positive_feat = seg_proto[i].unsqueeze(0).unsqueeze(0).repeat(self.num_queries, 1, 1)
                    uncertainty_positive_weigh = torch.ones(positive_feat.shape[:2]).unsqueeze(-1).to(device)
                    all_feat = torch.cat((positive_feat, negative_feat), dim=1)
                    all_uncertainty_weigh = torch.cat((uncertainty_positive_weigh, negative_feat_uncertainty_weigh),
                                                      dim=1)

                seg_logits = torch.cosine_similarity(anchor_feat.unsqueeze(1), all_feat, dim=2)
                seg_logits_weighted = all_uncertainty_weigh.squeeze(-1) * seg_logits
                reco_loss = reco_loss + F.cross_entropy(seg_logits_weighted / self.temp,
