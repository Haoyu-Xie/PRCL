...
class AverageValueMeter(Meter):
    ...
    def add(self, pred_logits: torch.Tensor, label: torch.Tensor) -> None:
        ...
        '''
        assert pred_logits.shape.__len__() == 4, 'pred_logits must be 4 dim, but now is {} dim!'.format(pred_logits.shape.__len__())
    ...
    def summary(self) -> dict:
        ...
        for c in range(self.num_class):
            if c != 0:
                Dice_dct['DSC_{}'.format(c)] = np.array([self.value(i, mode='all') for i in range(len(self.dice))])[:, c].mean()
        return Dice_dct

class ConfMatrix(Meter):
    ...
    def summary(self) -> dict:
        ...
        for c in range(self.num_classes):
            if c != 0:
                mIOU_dct['mIOU_{}'.format(c)] = np.array([self.value(i, mode='all')[0] for i in range(len(self.mIOU))])[:, c].mean()
                Acc_dct['Acc_{}'.format(c)] = np.array([self.value(i, mode='all')[1] for i in range(len(self.mIOU))])[:, c].mean()
