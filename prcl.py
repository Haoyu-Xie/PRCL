...
from generalframeworks.loss.loss import Attention_Threshold_Loss, Prcl_Loss
...
def main():
    ...
    criterion = {'ce_loss': nn.CrossEntropyLoss(ignore_index=-1).cuda(local_rank),
                 'unsup_loss': Attention_Threshold_Loss(strong_threshold=config['Prcl_Loss']['un_threshold']).cuda(local_rank),
                 'prcl_loss': Prcl_Loss(strong_threshold=config['Prcl_Loss']['strong_threshold'],
    ...
    for epoch in range(start_epoch, total_epoch):
        train(train_l_loader, train_u_loader, model, rank, local_rank, world_size, optimizer, optimizer_uncer, criterion, epoch, lr_scheduler, lr_scheduler_uncer, sche_d, config, args)
...
def train(train_l_loader, train_u_loader, model, rank, local_rank, world_size, optimizer, optimizer_uncer, criterion, epoch, scheduler, scheduler_uncer, sche_d, config, args):
