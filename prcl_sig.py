...
from generalframeworks.networks.ddp_model import Model_with_un_single
from generalframeworks.loss.loss import Attention_Threshold_Loss, Prcl_Loss_single
...
def main(args):
    ...
    if config['Dataset']['name'] == 'VOC':
        data = VOC_BuildData(data_path=config['Dataset']['data_dir'], txt_path=config['Dataset']['txt_dir'], 
                          label_num=config['Dataset']['num_labels'], seed=config['Seed'])
    ...
    train_l_sampler = torch.utils.data.RandomSampler(train_l_dataset)
    train_l_loader = torch.utils.data.DataLoader(train_l_dataset, 
                                                 batch_size=config['Dataset']['batch_size'],
                                                 pin_memory=True,
                                                 sampler=train_l_sampler,
                                                 num_workers=4)
    train_u_sampler = torch.utils.data.RandomSampler(train_u_dataset)
    ...
    ##### Loss init #####
    criterion = {'ce_loss': nn.CrossEntropyLoss(ignore_index=-1).cuda(),
                 'unsup_loss': Attention_Threshold_Loss(strong_threshold=config['Prcl_Loss']['un_threshold']).cuda(),
                 'prcl_loss': Prcl_Loss_single(strong_threshold=config['Prcl_Loss']['strong_threshold'],
                                        num_queries=config['Prcl_Loss']['num_queries'],
                                        num_negatives=config['Prcl_Loss']['num_negatives'],
                                        temp=config['Prcl_Loss']['temp']).cuda()
                }

    ##### Other init #####
    optimizer = torch.optim.SGD(model.model.parameters(), lr=float(config['Optim']['lr']), weight_decay=float(config['Optim']['weight_decay']),
                          momentum=0.9, nesterov=True)
    optimizer_uncer = torch.optim.SGD(model.uncer_head.parameters(), lr=float(config['Optim']['uncer_lr']), weight_decay=float(config['Optim']['weight_decay']),
                          momentum=0.9, nesterov=True)
    ...
    if os.path.exists(args.resume):
        ...
        model.uncer_head.load_state_dict(checkpoint['uncer_head'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        optimizer_uncer.load_state_dict(checkpoint['optimizer_uncer'])
        lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
        lr_scheduler_uncer.load_state_dict(checkpoint['lr_scheduler_uncer'])
    ...
    model.uncer_head.train()
    for epoch in range(start_epoch, total_epoch):
        train(train_l_loader, train_u_loader, model, optimizer, optimizer_uncer, criterion, epoch, lr_scheduler, lr_scheduler_uncer, sche_d, config)
        miou = test(test_loader, model.ema_model, config)
            ...
            torch.save(
                {
                    ...
                    'uncer_head': model.uncer_head.state_dict(),
...
def train(train_l_loader, train_u_loader, model, optimizer, optimizer_uncer, criterion, epoch, scheduler, scheduler_uncer, sche_d, config):
    ...
    iter_num = int(2000 / config['Dataset']['batch_size']/ len(train_l_loader)) #2000 img in a epoch
    ...
    for iter_i in range(iter_num):
        ...
        for i, (train_l_image, train_l_label) in enumerate(train_l_loader):
            ...
            prcl_loss = criterion['prcl_loss'](rep_all, uncer_all, label_all, mask_all, prob_all)
            total_loss = sup_loss + unsup_loss + prcl_loss * sche_d.value
...
def test(test_loader, model, config):
    ...
    miou = mean_intersection_over_union(miou_meter.mat) 
