...
if __name__ == '__main__':
    ...
    for epoch in range(max_epoch):
        ...
        for i in train_iter_tqdm:
            ...
            with torch.no_grad():
                pseudo_unlab_logits_0, _ = model_0(unlab_image)
                pseudo_unlab_logits_0 = F.interpolate(pseudo_unlab_logits_0, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)
                pseudo_unlab_porb_0, pseudo_unlab_label_0 = torch.max(torch.softmax(pseudo_unlab_logits_0, dim=1), dim=1)

                pseudo_unlab_logits_1, _ = model_1(unlab_image)
                pseudo_unlab_logits_1 = F.interpolate(pseudo_unlab_logits_1, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)
                pseudo_unlab_porb_1, pseudo_unlab_label_1 = torch.max(torch.softmax(pseudo_unlab_logits_0, dim=1), dim=1)

                # Augmentation
                unlab_image_aug_0, pseudo_unlab_label_0, pseudo_unlab_prob_0 = batch_transform(unlab_image, pseudo_unlab_label_0, pseudo_unlab_porb_0,
                                                                                crop_size=(256, 256), scale_size=(0.5, 1.5))
                unlab_image_aug_1, pseudo_unlab_label_1, pseudo_unlab_prob_1 = batch_transform(unlab_image, pseudo_unlab_label_1, pseudo_unlab_porb_1,
                                                                                crop_size=(256, 256), scale_size=(0.5, 1.5))
                # Apply mixing strategy: cutout, cutmix or classmix
                unlab_image_aug_0, pseudo_unlab_label_0, pseudo_unlab_prob_0 = generate_cut(unlab_image_aug_0, pseudo_unlab_label_0, pseudo_unlab_prob_0,
                                                                                            mode='cutout')
                # Generated from model_0, prepared for model_1
                unlab_image_aug_1, pseudo_unlab_label_1, pseudo_unlab_prob_1 = generate_cut(unlab_image_aug_1, pseudo_unlab_label_1, pseudo_unlab_prob_1,
                                                                                            mode='cutout')
                # Generated from model_1, prepared for model_0                                                                                         
                                                                                        
            # Preditions (with gradient)
            pred_sup_0, _ = model_0(lab_image)
            pred_sup_0 = F.interpolate(pred_sup_0, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)
            pred_sup_1, _ = model_1(lab_image)
            pred_sup_1 = F.interpolate(pred_sup_1, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)

            pred_unsup_0, _ = model_0(unlab_image_aug_1)
            pred_unsup_0 = F.interpolate(pred_unsup_0, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)
            pred_unsup_1, _ = model_1(unlab_image_aug_0)
            pred_unsup_1 = F.interpolate(pred_unsup_1, size=lab_label.shape[1: ], mode='bilinear', align_corners=True)

            _, pseudo_lab_label_0 = torch.max(torch.softmax(pred_sup_0, dim=1), dim=1)
            _, pseudo_lab_label_1 = torch.max(torch.softmax(pred_sup_1, dim=1), dim=1)
            
            pseudo_from_0 = torch.cat([pseudo_lab_label_0, pseudo_unlab_label_0], dim=0).long()
            pseudo_from_1 = torch.cat([pseudo_lab_label_1, pseudo_unlab_label_1], dim=0).long()

            pred_0 = torch.cat([pred_sup_0, pred_unsup_0], dim=0)
            pred_1 = torch.cat([pred_sup_1, pred_unsup_1], dim=0)
            
            # Generate pseudo labels
            #_, pseudo_label_0 = torch.max(pred_0, dim=1)
            #_, pseudo_label_1 = torch.max(pred_1, dim=1)
            #pseudo_label_0, pseudo_label_1 = pseudo_label_0.long(), pseudo_label_1.long()
            
            # Cross supervise
            cps_loss_0 = F.cross_entropy(pred_0, pseudo_from_1, reduction='mean', ignore_index=-1)
            cps_loss_1 = F.cross_entropy(pred_1, pseudo_from_0, reduction='mean', ignore_index=-1)
            # Supervised learning 
            sup_loss_0 = F.cross_entropy(pred_sup_0, lab_label, reduction='mean', ignore_index=-1)
            sup_loss_1 = F.cross_entropy(pred_sup_1, lab_label, reduction='mean', ignore_index=-1)
            ...
            lab_dice_0.add(pred_sup_0, lab_label)
            lab_dice_1.add(pred_sup_1, lab_label)
            unlab_dice_0.add(pred_unsup_0, unlab_label)
            unlab_dice_1.add(pred_unsup_1, unlab_label)
