...
from generalframeworks.loss.loss import Reco_Loss, Attention_Threshold_Loss
from generalframeworks.augmentation.transform import generate_cut
from generalframeworks.utils import label_onehot, fix_all_seed, tqdm_
...
def main(rank, config):
    ...
    model = DeepLabv3Plus(models.resnet101(pretrained=True), num_classes=data_loader.num_segments, output_dim=256)
    ...
    reco_loss = Reco_Loss(strong_threshold=config['Reco_Loss']['strong_threshold'], temp=config['Reco_Loss']['temp'],
                          num_queries=config['Reco_Loss']['num_queries'], num_negatives=config['Reco_Loss']['num_negatives']).cuda()
    ...
    for index in range(total_epoch):
        ...
        for i in train_epoch_tqdm:
            ...
            if config['Reco_Loss']['is_available']:
                ...
                contrastive_loss = reco_loss(rep_all, label_all, mask_all, prob_all)
            else:
